...
module.exports = function (pgWrapper) {
            ...
            await pgWrapper.transaction(async (client) => {
                const result = await client.query(`
                    SELECT sensors.sensor_id,
                           sensors.sensor_name,
                           array_agg(json_build_object('type_id', mt.type_id, 'measurement_formula',
                                                       sm.measurment_formula)) AS measurement_types
                    FROM sensors
    ...
    router.get('/:id', async (req, res) => {
        ...
        try {
            const {rows} = await pgWrapper.query('SELECT * FROM sensors WHERE sensor_id = $1', [id]);
            res.json(rows[0]);
        } catch (err) {
    ...
    router.delete('/:id', async (req, res) => {
        ...
        try {
            await pgWrapper.transaction(async (client) => {
                const {rows} = await client.query("select * from  sensors_measurements join measurements_type mt on sensors_measurements.type_id = mt.type_id join measurements m on mt.type_id = m.measuremnet_type where sensor_id=$1", [id]);
                console.log(rows)
                if (rows.length !== 0) return res.status(300).send("Измерения есть");
                await client.query('DELETE FROM sensors_measurements WHERE sensor_id = $1', [id]);
    ...
    router.get('/:id/type', async (req, res) => {
        const id = req.params.id;
        try {
            const {rows} = await pgWrapper.query('SELECT mt.type_id, sm.measurment_formula, mt.type_name, mt.type_units FROM sensors_measurements sm JOIN measurements_type mt ON mt.type_id = sm.type_id WHERE sensor_id = $1', [id]);
            res.json(rows);
        } catch (err) {
            res.status(500).json({error: err.message});
        }
